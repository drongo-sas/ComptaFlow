...
import { Lock, CheckCircle2, ChevronDown, Search } from "lucide-react";
...
export function TransactionDrawer({ tx, onClose, onSave }: Props) {
  ...
  // Reset local state whenever we open a different transaction
  useEffect(() => {
    if (tx) {
      ...
      <div className="fixed inset-y-0 right-0 z-50 flex w-full max-w-[680px] flex-col border-l bg-background shadow-2xl">
        ...
        <div className="flex shrink-0 items-center justify-between border-b bg-card px-6 py-4">
          <div className="min-w-0">
            ...
            <h2 className="mt-0.5 truncate text-base font-semibold">{tx.label}</h2>
          </div>
  ...
  return (
    <div className="space-y-1.5">
      ...
      <div className="flex h-10 items-center gap-2 rounded-lg border border-input bg-muted/40 px-3">
        <span className={cn("flex-1 truncate text-sm font-medium", valueClass)}>
          {value}
        </span>
